    ...
    const StonksVisuals = {
        ...
        init: function() {
            try {
                ...
                // RESPONSIVE TEXT CLASSES (text-3xl on mobile, text-6xl on desktop)
                this.textOverlay.innerHTML = '<h1 class="text-3xl md:text-6xl font-black text-white drop-shadow-lg text-center px-4" id="stonk-main-text"></h1><h2 class="text-xl md:text-4xl font-bold text-yellow-400 mt-2 md:mt-4 text-center px-4" id="stonk-sub-text"></h2>';
                ...
                // 1. Roles Panel (Top Left) - RESPONSIVE STYLING
                this.rolePanel = document.createElement('div');
                // Added md: prefixes for larger screens, default (mobile) is smaller
                this.rolePanel.className = 'absolute top-2 left-2 md:top-6 md:left-6 flex flex-col gap-1 md:gap-3 bg-slate-900/80 p-2 md:p-4 rounded-xl border border-white/10 backdrop-blur-md shadow-2xl min-w-[140px] md:min-w-[200px] transition-all';
                this.hudContainer.appendChild(this.rolePanel);

                // 2. Score Panel (Top Center) - RESPONSIVE STYLING
                this.scorePanel = document.createElement('div');
                // Compact on mobile, wider on desktop
                this.scorePanel.className = 'absolute top-2 md:top-6 left-1/2 -translate-x-1/2 flex gap-2 md:gap-6 bg-slate-900/80 p-2 md:p-4 rounded-full border border-white/10 backdrop-blur-md shadow-2xl max-w-[95vw] overflow-x-auto transition-all';
        ...
        updateHUD: function(players, roles) {
            ...
            // --- UPDATE ROLES ---
            let roleHTML = '<h3 class="text-gray-400 font-bold text-[10px] md:text-xs uppercase tracking-widest mb-1 border-b border-gray-700 pb-1 md:pb-2">Active Roles</h3>';
            ...
            Object.keys(roles).forEach(peerId => {
                ...
                if (role !== 'SPECTATOR') {
                     ...
                     roleHTML += `<div class="${color} font-bold text-sm md:text-base flex items-center gap-1 md:gap-2">
                        <span class="text-lg md:text-xl">${icon}</span> 
                        <span class="truncate max-w-[100px] md:max-w-[150px]">${player.name}</span>
                        <span class="text-[10px] md:text-xs text-gray-500 ml-auto uppercase tracking-tighter border border-gray-700 px-1 rounded">${role}</span>
                     </div>`;
                }
            });

            if (!activeRolesFound) roleHTML += '<div class="text-gray-500 text-xs md:text-sm italic">Waiting for round...</div>';
            ...
            sortedPlayers.slice(0, 5).forEach(p => {
                ...
                scoreHTML += `<div class="flex flex-col items-center w-12 md:w-20">
                    <div class="avatar-blob w-6 h-6 md:w-8 md:h-8 text-[10px] md:text-sm mb-0.5 md:mb-1 border-2 ${ringClass}">${p.avatar}</div>
                    <span class="text-[10px] md:text-xs text-gray-400 truncate w-full text-center font-bold">${p.name}</span>
                    <span class="text-sm md:text-lg font-display text-white">${p.score}</span>
                </div>`;
            });
